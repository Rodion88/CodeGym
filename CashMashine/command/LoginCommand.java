...
class LoginCommand implements Command {
	...
	public void execute() throws InterruptOperationException {
		...
		while(true) {
			...
			String pinCodeNumber = ConsoleHelper.readString();

			if(creditCardNumber == null || creditCardNumber.trim().length() != 12
					|| pinCodeNumber == null || pinCodeNumber.trim().length()!= 4) {
				ConsoleHelper.writeMessage(res.getString("try.again.with.details"));
			}else {
				try {
					if(validCreditCards.containsKey(creditCardNumber) && pinCodeNumber.equals(validCreditCards.getString(creditCardNumber))){
